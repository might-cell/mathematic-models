...
%% 在下面的代码中，我们先计算了权重，然后再进行了一致性检验，这是为了顺应计算过程，事实上在逻辑上是说不过去的，因为在进行推算权重的时候，有一种方法叫做特征值法，顺便先求一下特征值，这样方便一点。
%% 因此大家自己写论文中如果用到了层次分析法，一定要先对判断矩阵进行一致性检验。
%% 而且要说明的是，只有非一致矩阵的判断矩阵才需要进行一致性检验，如果我们构造的判断矩阵就是一个一致性矩阵，那样不需要进行检验，但还有有一点瑕疵比较好。
